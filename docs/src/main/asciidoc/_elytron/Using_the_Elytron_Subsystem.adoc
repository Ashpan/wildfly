...
It is possible to encrypt the filesystem realm by configuring the credential-store and secret-key attributes for the realm.

When creating a an encrypted filesystem realm, you will have to specify two more attributes.
...
* `secret-key`: This attribute specifies the alias to the secret key stored in the credential store. By default the alias in the credential store is set to `key`

To create a the credential store and secret key for the attributes above, you can create a secret-key-credential-store using the folloing commands. This will create a credential store called `mycredstore` with the secret key alias `key`.
[source,options="nowrap"]
----
...
To create an encrypted filesystem realm with a credential-store called ``mycredstore`` and the secret-key alias ``key``, the following command would be used
