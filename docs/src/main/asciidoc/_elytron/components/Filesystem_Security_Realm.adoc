...
* `secret-key` specifies the alias of the secret key that is stored in the previously specified credential store. This attribute is optional. Set to `key` by default.
...
The credential-store being referenced is called `mycredstore` and the alias of the key in that credential-store is `key`. The `key` secret key will be used to encrypt and decrypt the filesystem realm.
...
To encrypt the filesystem realm programmatically, you can specify a SecretKey when instantiating the realm.
...
Class `org.wildfly.security.auth.realm.FileSystemSecurityRealm` is used to instantiate the realm. A builder FileSystemSecurityRealm builder is available at `org.wildfly.security.auth.realm.FileSystemSecurityRealmBuilder` to make the instantiating process easier.
